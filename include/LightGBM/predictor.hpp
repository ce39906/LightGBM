...
public:
  ...
  */
  ...
  std::string Predict(const char* sample, const char sep) {
    ...
    double val = 0.0f;
    while (*sample != '\0') {
      sample = Common::Atof(sample, &val);
      if (std::fabs(val) > kZeroThreshold || std::isnan(val)) {
        oneline_features.emplace_back(idx, val);
      }
    ...
    // predict
    return Predict(oneline_features);
  }
  
  std::string Predict(const std::vector<std::pair<int, double>>& oneline_features) {
    // predict
    std::vector<double> result(num_pred_one_row_);
    predict_fun_(oneline_features, result.data());
    auto str_result = Common::Join<double>(result, "\t");
  ...
  std::string Predict(const std::vector<double>& features) {
    std::vector<std::pair<int, double>> oneline_features;
    for (int idx = 0; idx < static_cast<int>(features.size()); idx++) {
        oneline_features.emplace_back(idx, features[idx]);
    }
  ...
  std::vector<std::string> BatchPredict(const std::vector<std::vector<double>>& batch_features) {
    const int batch_size = batch_features.size();
    std::vector<std::string> results(batch_size);
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < batch_size; i++) {
        results[i] = Predict(batch_features[i]);
    }
